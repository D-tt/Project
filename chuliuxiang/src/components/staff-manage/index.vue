  ...
  <div class="system">
    ...
    <el-table :data="tableData" @selection-change="getRows">    <!-- border="true" 设置表格的垂直方向的边框 -->
        ...
        <template slot-scope="scope">
          <span>{{getroleLable(scope.row.role)}}</span>
        </template>
      </el-table-column>
			<el-table-column align="center" prop="state" label="状态">
        <template slot-scope="scope">
          <span>{{getstateLable(scope.row.state)}}</span>
        </template>
			</el-table-column>
    </el-table>
    ...
    <el-dialog  :visible.sync="dialogVisible" width="500px" title="新增">
			<el-form ref="dialogForm" :model="dialogForm" label-width="80px" :rules="dialogFormrules" inline>
              ...
              <!-- <el-input v-model="dialogForm.role" placeholder="请输入角色"></el-input> -->
						</el-form-item>
					</el-col>
			</el-form>
...
export default {
  ...
  methods:{
    ...
    // 表格 角色
    getroleLable (value) {
     let currentObj = this.roleOptions.filter(item => {
